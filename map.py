...
if maps_data == "google":
    cur.execute(
        "SELECT id,title,rating,price,g_lat,g_lon,author,date_created,picture,selftext,image_ns,permalink, strftime('%d.%m.%Y', datetime(date_created, 'unixepoch')) as form_date FROM doener where g_lat not null order by date_created desc")
elif maps_data == "bing":
    cur.execute(
        "SELECT id,title,rating,price,lon,lat,author,date_created,picture,selftext,image_ns,permalink, strftime('%d.%m.%Y', datetime(date_created, 'unixepoch')) as form_date FROM doener order by date_created desc")
else:  # mixed
    cur.execute(
        "SELECT id, title, rating, price, CASE WHEN g_lat = 0 THEN lon ELSE g_lat END AS selected_lat, CASE WHEN g_lon = 0 THEN lat ELSE g_lon END AS selected_lon, author, date_created, picture, selftext, image_ns, permalink, strftime('%d.%m.%Y', datetime(date_created, 'unixepoch')) AS form_date FROM doener WHERE g_lat IS NOT NULL ORDER BY date_created DESC;")
...
def home():
    ...
    return render_template("index.html", rows=rows, prices_all_median=prices_all_median,
                           ...
                           prices_22_median=prices_22_median, prices_21_median=prices_21_median, now=now,
                           maps_data=maps_data)


if __name__ == '__main__':
    app.config['SERVER_NAME'] = 'r-doener.github.io'
    with app.app_context():
        rendered = render_template("index.html", rows=rows, prices_all_median=prices_all_median,
                                   prices_24_median=prices_24_median, prices_23_median=prices_23_median,
                                   prices_22_median=prices_22_median, prices_21_median=prices_21_median, now=now,
                                   maps_data=maps_data)
        file = open("index.html", "w")  # write mode
        file.write(rendered)
        file.close()
    # app.run(debug=True)
